...
3. Linux 托盘限制说明：二进制内置的托盘后端（X11/xorg）**不支持右键菜单**，左键单击图标可唤出窗口；请使用窗口内的"退出"按钮退出程序。源码运行时若安装了 `python3-gi` + AppIndicator，托盘菜单可用。WSL/WSLg 环境没有系统托盘，程序会自动检测并保持窗口常驻（属预期行为，无报错输出）。
